...
void EncryptionSing(string inputCalc, double* arrCountCalc, bool logicEncryptionSing = false, double* coding = 0) {
    ...
    for (size_t i = 0; i < inputCalc.length(); i++)
    {
        ...
        if ((inputCalc[i] == '+') || 
            ...
            (inputCalc[i] == ')') || 
            (inputCalc[i] == '^') || 
            (inputCalc[i] == 's' && inputCalc[i + 1] == 'q' && inputCalc[i + 2] == 'r' && inputCalc[i + 3] == 't') || 
            (inputCalc[i] == '\0')) {
            number = "";
            index++;
            ...
            if (logicEncryptionSing) {
                ...
                if (inputCalc[i] == 's' && inputCalc[i + 1] == 'q' && inputCalc[i + 2] == 'r' && inputCalc[i + 3] == 't') {
                    i += 3;
                }
            }
    ...
    for (size_t i = 0; i < inputCalc.length(); i++)
    {
        if ((inputCalc[i] == '+') || 
            ...
            if ((inputCalc[i + 1] == '+') || 
                ...
                (inputCalc[i + 1] == ')') || 
                (inputCalc[i + 1] == '^') || 
                (inputCalc[i + 1] == 's' && inputCalc[i + 2] == 'q' && inputCalc[i + 3] == 'r' && inputCalc[i + 4] == 't') || 
                (inputCalc[i + 1] == '\0')) {
                arr.length++;
            }
        }
    ...
    for (int i = indexStart; i < indexEnd; i++)
    {
        ...
        // Subtraction
            ...
            *lastCoding = *coding;
            ...
            if (arrCountCalc[i - 1] >= *coding + 10 || arrCountCalc[0] == *coding + 40) {
                *lastCoding = *coding;
...
int main()
{
    ...
    Calc(arr.CountCalc, 0, arr.length, &coding, &lastCoding, arr.length, &error); // Counting the whole expression

    if (error) {
        cout << "ERROR" << endl;
    }
    else {
        cout << endl << arr.CountCalc[0] << endl; // Display calculations
    }
    return 0;
}
