...
string CesareanCode::Decryption(string str, int key) {
    for (int i = 0; i < key; i++)
    {
        for (size_t j = 0; j < str.length(); j++)
        {
            if (str[j] != ' '){
                ...
                {
                    if (str[j] == cesareanLower[k]) {
                        str[j] = CodingDecryption(cesareanLower, str, k, j);
                        break;
                    }
                    else if (str[j] == cesareanUpper[k]) {
                        str[j] = CodingDecryption(cesareanUpper, str, k, j);
                        break;
                    }
                }
            }
...
char CesareanCode::CodingDecryption(string cesarean, string str, int indexCesarean, int indexString)
{
    if ((indexCesarean - 1) < 0) str[indexString] = cesarean[(cesarean.length() - indexCesarean - 1)];
    else str[indexString] = cesarean[indexCesarean - 1];
